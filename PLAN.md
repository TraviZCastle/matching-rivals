...
- Matching 已改为本地优先：六个词对加载后，点击判断、进度、错误反馈与计时均在浏览器即时完成；对手进度后台同步，完成时提交同一个冻结的 `durationMs` 供结果页和排行榜使用。
...
5. 正式成绩使用完成第六对时由单调时钟冻结的 `durationMs`；比赛计时器、结果页和排行榜显示同一个值，数据库接收时间只用于校验与审计。
6. 先选择一个中文词，再选择一个英文词。
7. 配对正确：两项锁定并使进度 +1。
...
房间成员、题目与最终结果的权威快照保存在 Postgres。进行中的 Match 判断和本人进度采用本地状态，对手进度以不阻塞交互的后台请求同步；客户端通过私有 Realtime Broadcast 合并房间变化。每个标签页的匿名 Auth 会话、当前房间 ID 与本轮计时锚点保存在 `sessionStorage`，因此同一浏览器的两个标签页也可作为两个玩家验收，并可在刷新后继续本轮计时。
...
- 只能修改自己允许的玩家状态。
- 开局、后台进度同步与完成校验使用 Postgres RPC；每次 Match 不等待网络响应。
- 完成 RPC 校验房间成员、轮次、六个题目 ID、无重复、时长范围和幂等 ID；服务端接收时间用于合理性校验，不会替换用户已经看到的正式用时。
