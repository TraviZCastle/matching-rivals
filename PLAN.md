...
- 下一阶段已进入本地验收：5 分钟房间有效期、单人练习、五类题集选择，以及首位完成即结束比赛。
- 本地验收使用仅在开发环境开启的浏览器本地后端；对应 Supabase 迁移已准备但尚未应用，功能代码使用独立开发分支，Vercel Production 尚未更新。
...
7. 本地验证 5 分钟期限、单人练习、题集选择和首位完成即结束。
8. 验收后应用下一阶段 Supabase 迁移，再推送并部署。
